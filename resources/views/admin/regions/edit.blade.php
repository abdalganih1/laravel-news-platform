<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('تعديل المنطقة:') }} {{ $region->name }} ({{ $region->governorate->name ?? 'N/A' }})
        </h2>
    </x-slot>
                 ...
                 <div class="flex-grow">
                    <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
                        <div class="p-6 bg-white border-b border-gray-200">
                            <h3 class="text-lg font-medium text-gray-900 mb-4 text-right">تعديل تفاصيل المنطقة</h3>
                             @if ($errors->any())
                            ...
                            <form action="{{ route('admin.regions.update', $region) }}" method="POST" class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <select name="governorate_id" id="governorate_id" required
                                        ...
                                        @foreach ($governorates as $governorate)
                                            {{-- Check old value first, then the current region's governorate --}}
                                            <option value="{{ $governorate->governorate_id }}" {{ old('governorate_id', $region->governorate_id) == $governorate->governorate_id ? 'selected' : '' }}>
                                                {{ $governorate->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="name" id="name" value="{{ old('name', $region->name) }}" required maxlength="150"
                                           class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-right">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="gps_coordinates" id="gps_coordinates" value="{{ old('gps_coordinates', $region->gps_coordinates) }}" maxlength="100" placeholder="مثال: 33.5104,36.2765"
                                           class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-right">
